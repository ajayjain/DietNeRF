...
Download `nerf_synthetic.zip` and unzip it in the `dietnerf/data/` subdirectory.

## Starter commands (synthetic lego blender scene)

Feel free to add CUDA_VISIBLE_DEVICES=n to select the GPU to use. You need to add the CLIP repo to your PYTHONPATH, e.g. with `export PYTHONPATH=path/to/CLIP:$PYTHONPATH` or prepend `PYTHONPATH=path/to/CLIP:$PYTHONPATH` to each command. Run commands from `dietnerf/`.
